...
2. Confirm the current route in `CURRENT_STATE.md` matches `implementation-authorization-review` before implementation is authorized, or `loop-orchestration-implementation` after authorization is recorded; otherwise record a route mismatch in `review.md`.
