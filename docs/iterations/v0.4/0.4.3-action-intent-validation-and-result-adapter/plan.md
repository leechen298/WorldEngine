...
2. Confirm the current route in `CURRENT_STATE.md` is `implementation-authorization-review` before authorization, then `action-validation-implementation` after `review.md` records `implementation_authorized: yes`; otherwise record a route mismatch in `review.md`.
