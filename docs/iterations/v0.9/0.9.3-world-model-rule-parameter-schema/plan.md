...
Modify only after documentation review or implementation closeout:
...
Implementation candidate files after review authorization:
...
2. Run documentation existence, required-term, and Markdown cleanliness checks.
3. Request read-only subagent review for contract/design/test-plan coverage.
4. Fix any P0/P1/P2 findings before authorization.
5. If review passes, update package and parent docs to authorize only the
   reviewed non-live implementation scope.
6. Use `worldengine-iteration-dev` for implementation after authorization.
7. Run focused tests and backend regression.
8. Update package and parent review evidence with exact commands, changed
   files, compatibility review, scope review, unresolved findings, and next
   route.
