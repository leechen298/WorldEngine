...
Final search 可能只在明确的 "does not claim" boundary text 中命中 forbidden phrases。
任何 positive claim 都是 blocking。
...
- external validation。
- 除已记录 evidence checks 外，不运行 runtime、API、schema、checker、fixture 或 frontend tests。
