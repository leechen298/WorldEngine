...
fixture，不是 runtime world，也不是 game implementation。

## User Outcome

未来维护者应该能够查看一个小型、确定性的 world spec，理解 valid recursive `WorldSpec` 的
预期形状，而不需要启动 runtime engine、添加 loader 或解释 game behavior。

## Engineering Outcome

通过 review approval 后，implementation 应新增：

- 单个 JSON fixture：`backend/data/world_specs/tiny_village.world.json`。
...
- v0.7 或更晚 roadmap work 可以把 reference-village ideas 发展为完整 reference world 或
...
- 不实现 runtime bridge。
- 不让 Tiny Village 在 0.2.4 变成 runnable。
- 不增加 village runtime、game-specific backend logic、world generation、agent memory、
