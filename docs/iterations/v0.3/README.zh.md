...
- `0.3.0`：建立 v0.3 边界和兼容性门禁。
- `0.3.1`：定义 WorldSpec 加载器必须接受、返回和拒绝什么。
- `0.3.2`：让核心代码加载并校验通用 WorldSpec 数据。
- `0.3.3`：定义已加载的世界数据如何安全进入运行时上下文。
- `0.3.4`：让运行时持有可选世界上下文，同时不破坏旧行为。
- `0.3.5`：让外部验证样例运行器只消费公开契约。
- `0.3.6`：证明加载器和桥接层兼容，并可交接给 v0.4。
- `0.3.7`：让 v0.3 发布候选可以被评审。
- `0.3.8`：在评审批准后最终收口 v0.3。
