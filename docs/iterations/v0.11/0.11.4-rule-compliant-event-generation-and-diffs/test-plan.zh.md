...
- 聚焦后端测试通过。
- 有 accepted rules 的 session 可以 produce/apply 一个 legal public event/diff。
- 没有 accepted rules 的 session 返回公开 blocked/not-ready evidence。
- Direction-influenced candidates 包含 public direction refs，且不回显原始 instruction。
- Lightning-risk guidance 只影响 public pressure/probability evidence，绝不直接造成 Agent injury/death。
- Rejected/illegal candidates 不改变 public state。
- Accepted events 和 rejected/blocked evidence 可从 public event records replay。
- 现有 manual world evolution endpoint 保持兼容。
...
- Autonomous Agent validation 不属于 v0.11.4 范围。
...
`review.md` 只能记录当前 session 实际运行过的命令。不得从本包声明 provider live、外部验证、frontend E2E、自主 Agent、worldview fidelity、v0.11 PASS 或完整 MVP PASS。
