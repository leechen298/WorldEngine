...
1. Read v0.11 route, parent plan, iteration rules, existing session APIs, and
   existing world-direction classifier/tests.
...
6. If the evaluator passes, update `review.md` with
   `implementation_authorized: yes`; provider live and external validation stay
   `no`.
7. Implement only the approved additive session direction queue scope.
...
- Closeout begins only after focused verification and required evaluator
  checkpoints.
...
- change event generation, diff application, provider calls, Validation
  Client, persistence, migrations, frontend, or `backend/worldengine/`.
- let natural language directly impose final facts.
- expose raw instruction text, private markers, secrets, hidden context, raw
  prompts, raw responses, or provider traces.
