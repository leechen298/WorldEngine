...
def _public_surfaces() -> list[PublicSurface]:
    return [
        PublicSurface(
            ...
            notes=["basic service health surface"],
        ),
        PublicSurface(
            path="/openapi.json",
            method="GET",
            ...
            status="available",
            validation_status="pass",
            notes=["public API discovery surface"],
        ),
        PublicSurface(
            path="/manifest",
            method="GET",
            ...
            notes=["v0.10 MVP debug handoff entrypoint"],
        ),
        PublicSurface(
            path="/worlds",
            method="POST",
            ...
            required_for_mvp=True,
            notes=[
                "legacy public world creation surface; session APIs are implemented as separate MVP surfaces"
            ],
        ),
        PublicSurface(
            ...
            status="available",
            validation_status="pass",
            notes=["legacy external guidance surface"],
        ),
        PublicSurface(
            ...
            status="available",
            validation_status="pass",
            notes=["rule-bound world direction queue surface"],
        ),
        PublicSurface(
            path="/sessions/{session_id}/directions",
            method="POST",
            operation_id="submit_world_session_direction",
            status="available",
            validation_status="pass",
            notes=["session-scoped natural-language direction queue surface"],
        ),
        PublicSurface(
            path="/sessions/{session_id}/directions",
            method="GET",
            ...
            status="available",
            validation_status="pass",
            notes=["session-scoped natural-language direction summary surface"],
        ),
        PublicSurface(
        ...
        PublicSurface(
            path="/sessions/{session_id}/agents/{agent_id}",
            method="GET",
            ...
            validation_status="pass",
            required_for_mvp=False,
            notes=["session-scoped public Agent state read surface"],
        ),
        PublicSurface(
        ...
        PublicSurface(
            ...
            status="available",
            validation_status="pass",
            notes=["event legality evaluation surface"],
        ),
        PublicSurface(
            ...
            status="available",
            validation_status="pass",
            notes=["public Agent continuity evaluation surface"],
        ),
        PublicSurface(
            ...
            status="available",
            validation_status="pass",
            notes=["read-only narrative projection surface"],
        ),
        PublicSurface(
            ...
            status="available",
            validation_status="pass",
            notes=["read-only diagnostic dialogue evaluation surface"],
        ),
        PublicSurface(
            ...
            notes=["available API; live PASS depends on provider credentials and current evidence"],
        ),
        PublicSurface(
            path="/provider/worldview-preflight",
            method="POST",
            operation_id="provider_worldview_preflight",
            status="available",
            maturity="implemented",
            validation_status="pass",
            notes=["redaction-safe non-live provider/worldview preflight surface"],
        ),
        PublicSurface(
            ...
            status="available",
            validation_status="blocked",
            notes=["available API; provider-backed quality PASS is not claimed by manifest"],
        ),
        PublicSurface(
            path="/sessions",
            method="POST",
            operation_id="create_world_session",
            status="available",
            maturity="implemented",
            validation_status="pass",
            required_for_mvp=True,
            notes=["implemented in 0.10.2 as in-memory session create surface"],
        ),
        PublicSurface(
            path="/sessions/from-worldview",
            method="POST",
            ...
            validation_status="pass",
            required_for_mvp=True,
            notes=["implemented in 0.10.3 as redacted worldview-to-session creation"],
        ),
        PublicSurface(
            path="/sessions",
            method="GET",
            operation_id="list_world_sessions",
            status="available",
            maturity="implemented",
            validation_status="pass",
            required_for_mvp=True,
            notes=["implemented in 0.10.2 as in-memory session list surface"],
        ),
        PublicSurface(
            path="/sessions/{session_id}",
            method="GET",
            operation_id="get_world_session",
            status="available",
            maturity="implemented",
            validation_status="pass",
            required_for_mvp=True,
            notes=["implemented in 0.10.2 as in-memory session read surface"],
        ),
        PublicSurface(
            ...
            validation_status="pass",
            required_for_mvp=True,
            notes=["implemented in 0.10.2 as in-memory session status surface"],
        ),
        PublicSurface(
            path="/sessions/{session_id}/rules",
            method="POST",
            operation_id="attach_world_session_rules",
            status="available",
            maturity="implemented",
            validation_status="pass",
            notes=["session-scoped public rule/parameter attachment surface"],
        ),
        PublicSurface(
            path="/sessions/{session_id}/rules",
            method="GET",
            ...
            maturity="implemented",
            validation_status="pass",
            notes=["session-scoped public rule/parameter summary surface"],
        ),
        PublicSurface(
            path="/sessions/{session_id}/run",
            method="POST",
            ...
            validation_status="pass",
            required_for_mvp=True,
            notes=["implemented in 0.10.4 as bounded session runtime surface"],
        ),
        PublicSurface(
            ...
            validation_status="pass",
            required_for_mvp=True,
            notes=["implemented in 0.10.4 as bounded session runtime control"],
        ),
        PublicSurface(
        ...
        PublicSurface(
            path="/sessions/{session_id}/snapshots",
            method="GET",
            ...
            validation_status="pass",
            required_for_mvp=True,
            notes=["implemented in 0.10.4 as public session snapshot evidence"],
        ),
    ]


def _status_taxonomy() -> list[PublicStatusDefinition]:
    return [
        PublicStatusDefinition(
            status="pass",
            meaning="Current-session command or checker evidence passed for the declared scope.",
        ),
        PublicStatusDefinition(
            status="fail",
            meaning="Current-session evidence ran and found an in-scope failure.",
        ),
        PublicStatusDefinition(
            status="blocked",
            meaning="Validation or capability is blocked by missing environment, dependency, provider, or artifact support.",
        ),
        PublicStatusDefinition(
            status="not_run",
            meaning="The capability or validation layer is planned or discoverable but has not been executed.",
        ),
    ]


def _checker_handoff() -> CheckerHandoff:
    return CheckerHandoff(
        artifact_index=[
            CheckerArtifactRef(name="manifest.json", required_for_mvp=True, status="pass"),
            CheckerArtifactRef(name="operation-log.jsonl", required_for_mvp=True, status="not_run"),
            CheckerArtifactRef(name="api-summary.json", required_for_mvp=True, status="not_run"),
            CheckerArtifactRef(name="redaction-scan.json", required_for_mvp=True, status="not_run"),
            CheckerArtifactRef(name="scorecard-summary.json", required_for_mvp=True, status="not_run"),
        ],
        unsupported_items=[],
    )
...
def get_handoff_manifest() -> HandoffManifest:
    warnings = [
        "provider readiness is not live provider call proof",
        "session create/list/read/status surfaces are in-memory and not durable",
    ]
    blockers = []
    ...
    return HandoffManifest(
        ...
        checker_handoff=_checker_handoff(),
        blockers=blockers,
        warnings=warnings,
    )
